...
function TestValidator() {
  let list = new LinkedList<Rule<string[]>>({
    name: 'array',
    expect: input => `Expected value to be an <array> but received <${input.value}>`,
  ...
  const rules = {
    max: (value: any) => {
      list.append({
        name: 'max',
        expect: input => `Expected length of array to be less or equal than <${value}> but received <${input.value}>`,
